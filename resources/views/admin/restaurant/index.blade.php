...
@endsection
@section("subtitle")
يمكنك اضافة حذف وتعديل الشيفات
@endsection
...
<form class="row">
    ...
    <div class="col-sm-2">
        <a class="btn btn-success pull-right" href="/admin/restaurant/create">
            <i class="fa fa-plus"></i> اضافة شيف جديد</a>
    </div>
</form>
<br>
@if($items->count()>0)
<table class="table table-hover table-striped">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>{{$a->updated_at}}</td>
            <td>
                <a href="/admin/restaurant/{{$a->id}}/edit" class="btn btn-xs btn-primary">
                    <i class="glyphicon glyphicon-edit"></i>
                </a>
                <a href="/admin/restaurant/{{$a->id}}/delete" class="btn Confirm btn-xs btn-danger">
...
<script>
        $(function(){
            $(".cbStatus").click(function(){
                var id = $(this).val();
                // alert(id);
                $.get("/admin/restaurant/"+id+"/status");
            });
        });
    </script>
...
<script>
        $(function(){
            $(".allowcomment").click(function(){
                var id = $(this).val();
                $.get("/admin/restaurant/"+id+"/allowcomment");
            });
        });
    </script>
