...
@section("content")
<form action="/admin/restaurant" enctype="multipart/form-data" method="post" class="form-horizontal">
    {{csrf_field()}}
  <div class="form-group">
    <label for="title" class="col-sm-2 control-label">اسم الشيف </label>
    <div class="col-sm-10 col-md-5">
      <input autofocus type="text" class="form-control" value="{{old('title')}}" id="title" name="title" placeholder="ادخل اسم الشيف">
    </div>
  </div>
  <div class="form-group">
    <label for="summary" class="col-sm-2 control-label">نبذة عن الشيف </label>
    <div class="col-sm-10 col-md-10">
        <textarea  id="summary" rows="3" name="summary" placeholder="ادخل نبذة عن الشيف" class="form-control">{{old('summary')}}</textarea>
    </div>
  </div>
  <div class="form-group">
    <label for="details" class="col-sm-2 control-label">تفاصيل الشيف </label>
    <div class="col-sm-10 col-md-10">
        <textarea  id="details" rows="7" name="details" placeholder="ادخل تفاصيل الشيف " class="form-control">{{old('details')}}</textarea>
    </div>
  </div> 
