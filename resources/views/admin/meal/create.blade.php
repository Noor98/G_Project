...
@section("content")
<form action="/admin/meal" enctype="multipart/form-data" method="post" class="form-horizontal">
    {{csrf_field()}}
  <div class="form-group">
    <label for="title" class="col-sm-2 control-label">اسم الصنف </label>
    <div class="col-sm-10 col-md-5">
      <input autofocus type="text" class="form-control" value="{{old('title')}}" id="title" name="title" placeholder="ادخل اسم الصنف">
    </div>
  </div>
  ...
  <div class="form-group">
    ...
    <div class="col-sm-10 col-md-7">
        <textarea  id="details" rows="7" name="details" placeholder="ادخل مكونات الصنف " class="form-control">{{old('details')}}</textarea>
    </div>
  </div> 
    ...
    <div class="col-sm-10 col-md-5">
      <input autofocus type="number"  min="0" max="" step="0.1" class="form-control" value="{{old('price')}}" id="price" name="price" placeholder="ادخل سعر الصنف">
    </div>
  </div>
    ...
    <div class="col-sm-5">
        <select required name="restaurant_id" id="restaurant_id" class="form-control">
            ...
            @foreach($restaurants as $c)
                <option {{$c->id==old('restaurant_id')?"selected":""}} value="{{$c->id}}">{{$c->title}}</option>
            @endforeach
        </select>
    </div>
  </div>
